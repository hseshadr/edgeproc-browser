 ...
 */
export async function assertVectorIndexConformance(factory) {
    const checks = [
    ...
    if (failures.length > 0) {
        throw new Error(`VectorIndex conformance failed:\n- ${failures.join("\n- ")}`);
    }
}
async function checkBatchAtomicity(index) {
    await index.insert([record("existing", [1, 0, 0], {})]);
    await assertRejects(() => index.insert([
        record("would-be-partial", [0, 1, 0], {}),
        record("wrong", [1, 0], {}),
    ]), "insert accepted a batch containing the wrong dimension");
    assert((await index.read("would-be-partial")) === undefined, "a rejected insert batch partially published an earlier row");
    assert((await index.stats()).vectorCount === 1, "a rejected insert batch changed the live row count");
}
async function checkReplaceById(index) {
    ...
    assert((await index.stats()).vectorCount === 1, "replacement inserted a duplicate live row");
}
async function checkCopyIsolation(index) {
    ...
    assert(first.metadata.tenant === "original", "insert retained the caller's metadata object");
    first.vector[0] = 0;
    const second = await index.read("isolated");
    assert(second?.vector[0] === 1, "read() exposed the stored vector buffer");
}
async function checkRoundTrip(index) {
    ...
    assert((far.at(-1)?.distance ?? 0) >=
        (far[0]?.distance ?? Number.POSITIVE_INFINITY), "search results must use ascending, lower-is-closer distance");
}
async function checkAndFilters(index) {
    await index.insert(seedRows());
...
async function checkEmptyFilters(index) {
    ...
    assert((await index.stats({})).vectorCount === 4, "stats({}) must be unscoped");
}
async function checkScopedDelete(index) {
    await index.insert(seedRows());
    const deleted = await index.delete(["a-hot", "b-hot"], { tenant: "a" });
    assert(deleted === 1, "scoped delete must report only matching rows");
    assert((await index.read("a-hot")) === undefined, "matching row survived delete");
    assert((await index.read("b-hot")) !== undefined, "scoped delete crossed scope");
}
async function checkUnscopedDelete(index) {
    await index.insert(seedRows());
...
async function checkDimensionRefusal(index) {
    await assertRejects(() => index.insert([record("wrong", [1, 0], { tenant: "a" })]), "insert accepted the wrong dimension");
    await assertRejects(() => index.search(new Float32Array([1, 0]), 1), "search accepted the wrong dimension");
}
async function checkNonFiniteRefusal(index) {
    await assertRejects(() => index.insert([record("nan", [Number.NaN, 0, 0], {})]), "insert accepted NaN");
    await assertRejects(() => index.search(new Float32Array([Number.POSITIVE_INFINITY, 0, 0]), 1), "search accepted Infinity");
}
async function checkNonScalarMetadataRefusal(index) {
    const invalid = { nested: { unsafe: true } };
    await assertRejects(() => index.insert([record("nested", [1, 0, 0], invalid)]), "insert accepted nested metadata");
    await assertRejects(() => index.search(QUERY, 1, invalid), "search accepted a nested metadata filter");
}
async function checkInvalidLimitRefusal(index) {
    for (const limit of [-1, 1.5, Number.NaN]) {
        await assertRejects(() => index.search(QUERY, limit), `search accepted invalid limit ${String(limit)}`);
    }
}
async function checkDeterministicTies(index) {
    await index.insert([
        record("z-tie", [1, 0, 0], {}),
        record("a-tie", [1, 0, 0], {}),
    ]);
...
async function checkDisposedUseRefusal(index) {
    await index.dispose();
    await index.dispose();
...
function record(id, vector, metadata) {
    return { id, vector: new Float32Array(vector), metadata };
}
function assertIds(hits, expected) {
    const actual = hits.map(({ id }) => id);
    assert(actual.length === expected.length &&
        actual.every((id, index) => id === expected[index]), `expected ids ${JSON.stringify(expected)}, received ${JSON.stringify(actual)}`);
}
