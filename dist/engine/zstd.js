...
function readContentSize(view, offset, width) {
    if (width === 1) {
...
function parseFrameHeader(view) {
    ...
    const width = flag === 0 ? Number(singleSegment) : (FCS_FIELD_SIZES[flag] ?? 0);
    // magic(4) + descriptor(1) + Window_Descriptor(1, single-segment omits it).
    const offset = 5 + Number(!singleSegment) + (DICTIONARY_ID_SIZES[descriptor & 0b11] ?? 0);
...
function frameByteLength(view, header) {
 ...
 */
export function declaredContentSize(bytes) {
 ...
 */
export async function decompressBounded(bytes, expectedSize) {
